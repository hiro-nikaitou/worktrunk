...
When adding a column or preview, decide which data is needed for the skeleton and which can arrive later. Plain list is a run-to-completion command, so even progressive forge work delays exit; prefer picker-only live details where possible. A shell prompt must never touch the network.
